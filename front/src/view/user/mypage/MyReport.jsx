...
function MyReport(){
    ...
    const getBoardList = () => {
        if(user)
            axios.get(`${serverIP.ip}/mypage/reportList?nowPage=${nowPage}`,{
    ...
    return(
        <div className='report-box'>
            ...
            </ul>
            {
                ...
                report.map(item => {
                    return (<><ul className='mypage-report-list'>
                        <li>
                            {item.id}
                        </li>   
                        ...
                        <li>
                        <span className={`report-state ${item.state.toLowerCase()}`}>
                            {item.state === "READABLE" ? "처리 전" 
                            : item.state === "PROCESSING" ? "처리 중" 
                            : "처리 완료"}
                        </span>
                        </li>
                    </ul>
                    <div className='report-detail' id={'report-detail-' + item.id} style={{display:'none'}}>
                        <div className='report-date'>신고일: {item.createDate} &nbsp;&nbsp;&nbsp; 처리일:{item.endDate}</div>
